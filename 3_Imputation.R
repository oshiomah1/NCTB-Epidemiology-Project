...
imputed.dataset.1.filtered.a = imputed.dataset.1.dob2 %>% mutate(sex= case_when(sex == "1" ~ "Male", sex == "2" ~ "Female"))  %>% mutate(smokes = case_when(smokes== "1" ~ "Smoker", smokes == "0" ~ "Non-Smoker")) %>% mutate(drnk_alcohol = case_when(drnk_alcohol== "1" ~ "Drinker", drnk_alcohol == "0" ~ "Non-Drinker")) %>% mutate(diabetes = case_when(diabetes== "1" ~ "Diabetic", diabetes == "0" ~ "Non-Diabetic")) %>% mutate(across(c("sex","drnk_alcohol","smokes","diabetes","migration","cohort2"), as.factor))
